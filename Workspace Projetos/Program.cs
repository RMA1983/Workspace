...
using System.Text; //para usarmos File  //https://docs.microsoft.com/en-us/dotnet/api/system.io.file.readalltext?view=net-6.0
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if(File.Exists(pathFileInvestidor))
            {
                ...
                if (double.TryParse(fileContentSplit[0], out conteudoFicheiroDouble)) 
                { 
                    _investidor.EurosDepositados = conteudoFicheiroDouble;
                }
                if (int.TryParse(fileContentSplit[1], out conteudoFicheiroInteiro)) 
                { 
                    _investidor.TotalCHOW = conteudoFicheiroInteiro; 
                }
                if (int.TryParse(fileContentSplit[2], out conteudoFicheiroInteiro)) 
                { 
                    _investidor.TotalDOCE = conteudoFicheiroInteiro; 
                }
                if (int.TryParse(fileContentSplit[3], out conteudoFicheiroInteiro)) 
                { 
                    _investidor.TotalGALLO = conteudoFicheiroInteiro; 
                }
                if (int.TryParse(fileContentSplit[4], out conteudoFicheiroInteiro)) 
                { 
                    _investidor.TotalTUGA = conteudoFicheiroInteiro;
                }
            }
            ...
            switch (ReadLine())
            {
                case "A":
                    ...
                    while(showMenuInvestidor){
                        showMenuInvestidor = SubMenus.MenuInvestidor(_investidor, simulacao);
                    }
                    return true;
                case "B":
                    Clear();
